...
//      "UnusedPrivateField"
  ...
  public void processFile(String fileToCheck, RuleSet ruleset) {
    ...
    try {
      ...
      ctx.setSourceCodeFilename(glomName(opts.shortNamesEnabled(), fileToCheck, file));

        // file chechking....
        pmd.processFile(new FileInputStream(file), opts.getEncoding(), ruleset, ctx);
      } catch (Exception pmde) {
        if (opts.debugEnabled()) {
          pmde.getCause().printStackTrace();
        }
